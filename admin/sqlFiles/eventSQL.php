    ...
    if (isset($_POST["submit"]) && !empty($_FILES["file"]["name"])) {
        ...
        if (in_array($fileType, $allowTypes)) {
            if (move_uploaded_file($_FILES["file"]["tmp_name"], $targetFilePath)) {
                $query = "INSERT INTO `events`(`title`, `description`, `image`, `status`) VALUES ('$title','$description','" . $create_datetime . $fileName . "','$status')";
                ...
                if ($result) {
                    ...
                    echo "<script type='text/javascript'> document.location = '../new-event.php'; </script>";
                }
            } else {
